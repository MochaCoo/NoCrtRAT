...
var download_Option struct {
	CancelCompress bool `short:"c" long:"CancelCompression" description:"传输文件时取消压缩"`
}
...
var cmd_Option struct {
	...
	Timeout uint `short:"t" long:"time" default:"0" description:"最长等待时间"`
}

var FlagParser flags.Parser
func InitFlag(){
	cmd_Option.Wait=false
	download_Option.CancelCompress=false
	upload_Option.CancelCompress=false
	...
	FlagParser.AddCommand("lsf","列出当前目录下所有文件夹和文件,空路径时会列出所有盘符","列出当前目录下所有文件夹和文件,空路径时会列出所有盘符",&lsf_Option)
	FlagParser.AddCommand("cd","进入文件夹 cd空路径可清空当前路径","进入文件夹 cd空路径可清空当前路径",&cd_Option)
	...
	FlagParser.AddCommand("cmd","在当前工作目录执行命令 可选参数: -w控制是否读取回显,执行时间久的任务会导致客户端阻塞 -t配合-w使用,设置最长等待时间","在当前工作目录执行命令 可选参数: -w控制是否读取回显,执行时间久的任务会导致客户端阻塞 -t配合-w使用,设置最长等待时间",&cmd_Option)
}
